...
def test():
	...
	fname_list = []
	for _,_,f in os.walk(MIT_PATH):
		fname_list.extend(f)
	random.shuffle(fname_list)
	for i in range(int(sys.argv[2])):
		try:
			img = Image.open(MIT_PATH+'/'+fname_list[i])
			w, h = img.size
			img = np.array(img)
			# lbl = np.array(Image.open(LABEL_PATH+'/'+fname_list[i].split('.')[0]+'.jpeg').resize((OU_W,OU_H)))
			img = np.swapaxes(img,0,2)
			model.forward(Batch([mx.nd.array(img).reshape((1,3,w,h))]),is_train=False)
			pred = model.get_outputs()[0].asnumpy()[0][0]
			pred = 250*(pred-np.amin(pred))/(np.amax(pred)-np.amin(pred))
			Image.fromarray(pred.transpose().astype('uint8')).resize((w,h)).convert('L').filter(ImageFilter.SMOOTH_MORE).filter(ImageFilter.GaussianBlur(5)).save('../pred/'+fname_list[i].split('.')[0]+'_pred.jpg')
			# Image.fromarray(pred[0][0].transpose().astype('uint8')).resize((640,480)).save('../pred/'+fname_list[i].split('.')[0]+'_pred.jpg')
			Image.fromarray(np.swapaxes(img,0,2).astype('uint8')).resize((w,h)).save('../pred/'+fname_list[i])
			# Image.fromarray(lbl.astype('uint8')).resize((640,480)).save('../pred/'+fname_list[i].split('.')[0]+'_label.jpg')
